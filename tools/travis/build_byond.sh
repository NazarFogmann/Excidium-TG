...
if [ "$BUILD_TOOLS" = false ]; then
	...
	if [ "$BUILD_TESTING" = true ]; then
		tools/travis/dm.sh -DTRAVISBUILDING -DTRAVISTESTING -DALL_MAPS excidiumstation.dme
	else
		tools/travis/dm.sh -DTRAVISBUILDING excidiumstation.dme
		...
		DreamDaemon excidiumstation.dmb -close -trusted -verbose -params "test-run&log-directory=travis"
		cd ..
